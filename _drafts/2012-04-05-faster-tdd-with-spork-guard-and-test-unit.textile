...
enki_id: 5
categories: rails, guard, spork, test unit, tdd
---
...
p. Follow these steps to set up Spork & Guard in your Rails app with Test::Unit (steps can be easily adapted for RSpec).

h2. 1. Add the following gems to your Gemfile
...
h2. 2. Install and bootstrap guard and spork
...
h2. 3. Edit your Guardfile (in Rails.root)

*Note: The spork section needs to be above any other test sections in your Guardfile, because they are loaded in the order they are specified*

Add or remove watch expression as you need to (see the "guard docs":https://github.com/guard/guard for usage), but don't forget to add the @:test_unit_env => {'RAILS_ENV' => 'test'}@ option to the spork section and the @:drb => true@ option to the test section.

h3. Example Guardfile:

bc.. guard 'spork', :test_unit_env => { 'RAILS_ENV' => 'test' }, :wait => 60 do
...
h2. 4. Edit your test/test_helper.rb
...
Now as soon as you make changes to any watched files, the relevant tests will be re-run, super fast!

h3. Guard tips and tricks
